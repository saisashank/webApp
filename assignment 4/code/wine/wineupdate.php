...
<?php
					...
					die('cant use' . databaseName.':'.mysql_error());
					 ...
					 print "<br>FOLLOWING DATA IS UPDATED INTO RESPECTIVE TABLES :: <br><br>";
					   if ($wine_name !=NULL && $winery_name !=NULL ){
					 $query = "UPDATE wine, winery  SET wine.wine_name='{$wine_name}', winery.winery_name='{$winery_name}' WHERE wine.wine_id={$wine_id} AND winery.winery_id={$winery_id}";
				...
				print "<tr><td><p>{$row["winery_id"]}</p></td>" .
					...
					mysql_free_result($result5);
					  ...
					  if (!(@ mysql_query ($query1, $connection)))   showerror();
					  else
					  {
						  $message = "One record updated.";
